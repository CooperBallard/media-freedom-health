from helpers import load_panel, cluster_ols
 
#load merged panel (2000-2023, life-expectancy zeros recoded, logs added)
panel = load_panel()
...
f1 = cluster_ols("e_pelifeex ~ v2x_freexp_altinf + C(year)", f1d)
...
f2 = cluster_ols("e_pelifeex ~ v2x_freexp_altinf + log_gdppc + C(year)", f2d)
...
f3 = cluster_ols("e_pelifeex ~ v2x_freexp_altinf + log_gdppc + log_health_exp + C(year)", f3d)
...
f4 = cluster_ols("e_pelifeex ~ v2x_freexp_altinf + log_gdppc + log_health_exp + e_wbgi_rle + v2csreprss + C(year)", f4d)
...
p1 = cluster_ols("e_pelifeex ~ v2x_polyarchy + C(year)", p1d)
...
p2 = cluster_ols("e_pelifeex ~ v2x_polyarchy + log_gdppc + C(year)", p2d)
...
p3 = cluster_ols("e_pelifeex ~ v2x_polyarchy + log_gdppc + e_wbgi_rle + e_wbgi_cce + C(year)", p3d)
...
for k in [0, 1, 3, 5]:
    ...
    m = cluster_ols(f"e_pelifeex ~ {col} + log_gdppc + C(country_text_id) + C(year)", d)
